...
const init = async () => {
    ...
    if (sessionData.user_access) {
        ...
        // 네비게이션 바 유저 정보 업데이트
        setUserChallengesData(sessionData);
        // 로그인시 채팅서버 연결
        if (sessionData.user_access) {
            connectWebSocket();
        }
...
// 유저 챌린지 데이터 업데이트
const setUserChallengesData = async (sessionData) => {
    ...
    try {
        ...
        if (data.length > 0) {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                // 포인트 총합 출력
                n_name_el.textContent = sessionData.nickname;
...
main_menu_list_el.addEventListener('mouseenter', () => {
    ...
    // 이전 타이머가 있다면 취소
    if (menuAnimationTimer) clearTimeout(menuAnimationTimer);
    if (menuResizeTimer) cancelAnimationFrame(menuResizeTimer);


    main_menu_list_el.classList.replace('col-1', 'col-2'); // col-1을 col-2로 변경

    // col 클래스 변경 애니메이션이 완료된 후 내용 변경
...
main_menu_list_el.addEventListener('mouseleave', () => {
    ...
    // 내용 변경 후 크기 축소
    menuResizeTimer = requestAnimationFrame(() => {
        main_menu_list_el.classList.replace('col-2', 'col-1');
    });
});
